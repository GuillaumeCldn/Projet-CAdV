...
\begin{document}
	...
	Ce projet a permis de concevoir et valider une architecture de pilotage automatique complète pour la descente et la tenue de palier d'un Airbus A320. La démarche progressive, allant de la linéarisation à la validation non-linéaire, a permis de mettre en évidence plusieurs points clés de l'automatique aéronautique :
	
	\begin{itemize}
		\item \textbf{Gestion des non-linéarités :} La linéarisation autour d'un point de trim est une méthode efficace pour la synthèse, mais elle montre ses limites lors des phases de grande amplitude (Open Descent). L'ajout d'intégrateurs s'est révélé indispensable pour compenser les variations paramétriques (notamment la densité de l'air) et annuler l'erreur statique du modèle réel.

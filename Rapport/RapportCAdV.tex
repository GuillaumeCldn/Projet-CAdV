...
\begin{document}
	...
	Afin de valider la synthèse et la qualité de la linéarisation, nous avons superposé la réponse indicielle des deux modèles pour un échelon de consigne de \SI{5}{\degree} partant du point de trim.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.95\textwidth, trim={1cm 0.5cm 1cm 1cm}, clip]{reponse_bilineaire_assiette.png}
		\vspace{-1em} 
	...
	La linéarisation est effectuée sur le modèle Simulink \texttt{acDynModelGolfTenueClassique}, qui inclut déjà la boucle interne d'assiette. Le système linéarisé $A_6$ est donc d'ordre 6 et correspond au vecteur d'état suivant :
	\begin{equation}
		x = [V_a, \alpha, \theta, q, x_{pid}, x_{pf}]^T
	\end{equation}
	Où $x_{pid}$ et $x_{pf}$ sont les états internes du correcteur PI-D et du pré-filtre de la boucle interne.
	
	Afin d'annuler l'erreur statique, nous avons augmenté ce système avec un état intégrateur $\xi$ agissant sur l'erreur de vitesse équivalente :
	\begin{equation}
		\dot{\xi} = V_{EAS,cons} - V_{EAS,mes}
	\end{equation}
	La commandabilité du système augmenté a été vérifiée via le rang de la matrice de commandabilité (\texttt{ctrb}).
	
	\begin{figure}[H]
		\centering
		%\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{schema_prefiltre.png}}
		\caption{Schéma Simulink de descente avec préfiltre H}
	...
	Pour calculer le gain $K_{aug}$ du système augmenté, nous avons utilisé la commande \texttt{place} avec les pôles désirés suivants :
	\begin{itemize}
		\item \textbf{Dynamique dominante ($dp$)} : Conservée de la méthode de Wang (boucle interne).
		\item \textbf{Modes rapides} : Pôles complexes conjugués conservés vers $-3.4 \pm 0.15i$ (observés dans la matrice $A_6$ via \texttt{eig}). Nous avons choisi de ne pas modifier ces modes naturels pour éviter de solliciter inutilement les actionneurs.
		\item \textbf{Pôles réels de réglage} : Fixés à $-3$ et $-4$.
		\item \textbf{Pôle de l'intégrateur} : Fixé à $-10.33$.
	\end{itemize}
	Ce choix d'un pôle intégrateur rapide ($-10.33$) assure une élimination rapide de l'erreur statique, bien au-delà de la contrainte de stabilité imposée ($\text{Re} < -1.3$).
	...
	\end{equation}
	Où $\hat{x}$ est l'état estimé et $L$ la matrice de gain de l'observateur. Cet observateur est synthétisé en utilisant les trois sorties mesurées disponibles définies dans la matrice $C_{obs}$ :
	\begin{equation}
		y_{mes} = [V_a, \theta, q]^T
	\end{equation}
	L'observabilité de la paire $(A_6, C_{obs})$ a été validée (rang plein).

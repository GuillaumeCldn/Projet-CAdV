...
\begin{document}
	...
	Le choix du pôle intégrateur à $-1.33$ permet de satisfaire la spécification de stabilité ($\text{Re} < -1.3$)[cite: 804]. On note que la dynamique de l'intégrateur est du même ordre de grandeur que la dynamique de l'assiette ($1.33 \approx 1.33$), ce qui est une limite de cette configuration, mais nécessaire pour respecter les contraintes sans demander une dynamique irréaliste à l'actionneur.

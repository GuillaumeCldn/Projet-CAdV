...
\begin{document}
	...
	\vspace{0.5cm}
	Afin de valider la synthèse, nous avons superposé la réponse indicielle des deux modèles pour une consigne de \SI{5}{\degree}.
	
	\begin{figure}[htbp]
		\centering
		\begin{minipage}[c]{0.58\textwidth}
			\centering
			\includegraphics[trim={0 0 0 0.82cm}, clip, width=\linewidth]{reponse_bilineaire_assiette.png}
		...
		\begin{minipage}[c]{0.38\textwidth}
			...
			\begin{enumerate}[leftmargin=*] 
				...
				\item \textbf{Validité :} Superposition parfaite des courbes rouge et bleue. La linéarisation est valide pour les faibles amplitudes ($5^\circ$).
			\end{enumerate}
		\end{minipage}
	\end{figure}
	...
	La simulation de la commande modale avec intégrateur donne le résultat suivant :
	\begin{figure}[H]
		\centering
	...
	Cette réponse est satisfaisante : le système répond en environ \SI{3.5}{s} et avec un dépassement de l'ordre de \SI{6}{\%}. Nous avons donc implémenté cette commande sur le modèle non linéaire.
	
	\section{Modèle non linéaire}
	
	Le schéma suivant illustre l'intégration de la commande modale sur le système non linéaire. Nous avons conservé la possibilité d'alterner entre la précommande $H$ et l'intégrateur, même si l'intégrateur se révèle être supérieur. Il est aussi important de noter que l'intégrateur agit sur l'erreur d'EAS (et non de TAS), car c'est la grandeur pilotée.
	...
	Pour mettre en œuvre la commande, on linéarise le modèle avion au nouveau point de trim : \SI{230}{kts} et \SI{3000}{ft}. Cette linéarisation est réalisée par \texttt{utAcDynamicsFunction.m} et donne le point de fonctionnement suivant :
	\begin{equation}
	...
	\begin{figure}[H]
		...
		\caption{Réponse de la commande optimale appliquée au modèle linéaire à des échelons de pente de \SI{10}{\degree} et de vitesse de \SI{230}{kts}}
	...
	Le passage de la loi \textit{Open Descent} (vitesse tenue par l'assiette) aux lois de maintien (vitesse tenue par la poussée) constitue une phase critique du vol. La transition entre les lois de commande est faite bar un bloc \texttt{Switch} qui bascule lorsque l'atitude de l'avion passe sous la barre de \SI{914}{m} (\SI{3000}{ft}). Ceci implique que l'avion se stabilise environ \SI{50}{ft} sous son altitude cible. On peut corriger cela naïvement en règlant le seuil du bloc \texttt{Switch} (un seuil à \SI{17}{m} fonctionne), ou en implémentant un loi qui gère plus finement la capture de l'altitude.
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.4\textwidth}
			...
			\caption{Courbe de suivi de l'altitude}
			\label{fig:reponse_finale_ALT}
		\end{subfigure}
		\begin{subfigure}[b]{0.4\textwidth}
			\includegraphics[width=\textwidth]{reponse_EAS_400.png}
			\caption{Courbe de suivi de la vitesse.}
			\label{fig:reponse_finale_EAS_long}
		\end{subfigure}
		\begin{subfigure}[b]{0.4\textwidth}

...
Nous avons retenu une structure de commande classique de type PI-D (Proportionnel Intégral, Dérivée sur la mesure) associée à un pré-filtre sur la consigne. Les gains ont été calculés via la méthode de Wang, présentée dans le sujet au paragraphe 3.5.1 du sujet \cite{Sujet} et dans l'article de \emph{Wang et al.} \cite{Wang2009}.
	...
	Le sujet suggère l'ajout d'un intégrateur pour annuler l'erreur statique de vitesse \cite[3.6.2]{Sujet}. Cette approche est indispensable car le modèle linéaire varie fortement au cours de la descente.
	Nous avons augmenté le système avec un état intégrateur $\xi$ défini par :
	\begin{equation}
		\dot{\xi} = V_{\text{EAS,cons}} - V_{\text{EAS,mes}}
	\end{equation}
	Il est crucial de noter que l'intégrateur agit sur l'erreur d'EAS (et non de TAS), car c'est la grandeur pilotée.
	
	\begin{figure}[H]
		\centering
		%\makebox[\textwidth][c]{\includegraphics[width=1.2\textwidth]{schema_integrateur.png}}
		\caption{Schéma Simulink de descente avec l'intégrateur}
		\label{fig:modale_integrateur}
	\end{figure}
